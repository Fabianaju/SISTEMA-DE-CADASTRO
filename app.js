// app.js
import { getClientes, saveClientes, getFichas, saveFichas, deleteCliente, deleteFicha,
         getSession, signIn, signOut, onAuthChange } from './data.js';
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pr-footer-row .pr-fv { font-family: 'Courier New', monospace; font-weight: 700; color: #14213D; }
  .pr-obs-box {
    border: 1px solid #D8D2C4;
    border-radius: 6px;
    padding: 10px 16px;
...
<div class="pr-client-box">
  <div class="pr-cl-full">
    <span class="pr-cl-label">Paciente</span>
    ${esc(c ? c.nome : '—')}
  </div>
  <div>
    <span class="pr-cl-label">Telefone</span>
    ${esc(c ? c.telefone : '—')}
  </div>
  <div>
    <span class="pr-cl-label">Data de nascimento</span>
    ${c && c.nascimento ? fmtDate(c.nascimento) + (idade ? ' · ' + idade : '') : '—'}
  </div>
  <div>
    <span class="pr-cl-label">Data da consulta</span>
    ${fmtDate(f.data)}
  </div>
  <div>
    <span class="pr-cl-label">Oculista / Optometrista</span>
    Dr(a). ${esc(f.oculista)}
  </div>
</div>
    ...
    <div class="view login-view">
      <div class="login-card">
        <div class="login-logo-row">
          <img src="logo.png" alt="Óticas Precisão" class="login-logo">
        </div>
        <h2 class="login-title">Óticas Precisão</h2>
        <p class="login-sub">Acesso restrito a funcionários</p>
        ${errMsg ? `<div class="login-error">${errMsg}</div>` : ''}
        <form id="login-form">
          <div class="field">
            <label>E-mail</label>
            <input type="email" name="email" required placeholder="seu@email.com" autocomplete="username">
          </div>
          <div class="field">
            <label>Senha</label>
...
function showLogin(errMsg) {
  document.getElementById('app').innerHTML = renderLogin(errMsg);
  ...
  document.getElementById('login-form').addEventListener('submit', async (e) => {
    ...
    btn.disabled = true;
    try {
      await signIn(fd.get('email').trim(), fd.get('senha'));
      // onAuthChange dispara e chama loadAll automaticamente
    } catch (err) {
      const msg = err.message && err.message.includes('Invalid login')
        ? 'E-mail ou senha incorretos.'
        : 'Erro ao entrar. Tente novamente.';
      document.getElementById('app').innerHTML = renderLogin(msg);
      // Re-attach listener
      showLogin(null); // re-render handled by recursive call would loop — use direct reattach
    }
  });
}
...
function addLogoutButton(userName) {
  ...
  if (!topbar) return;
  let existing = topbar.querySelector('.logout-btn');
  if (existing) existing.remove();
  const btn = document.createElement('button');
  btn.className = 'btn btn-ghost logout-btn';
  btn.style.marginLeft = 'auto';
  btn.style.color = '#B9C2D6';
  btn.style.fontSize = '13px';
  btn.title = 'Sair da conta';
  btn.textContent = `Sair (${userName})`;
...
// Bootstrap: verifica sessão existente e reage a mudanças de auth
onAuthChange(async (session) => {
  if (session) {
    const email = session.user.email;
    const nome = email.split('@')[0];
    addLogoutButton(nome);
    if (!state.loaded) await loadAll();
  } else {
    // Sem sessão — limpa estado e mostra login
    state.clientes = [];
    state.fichas = [];
    state.loaded = false;
    state.view = 'list';
    // Remove botão logout se existir
...
// Verifica sessão inicial (para quem já estava logado)
