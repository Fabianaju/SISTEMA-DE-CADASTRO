...
// Bootstrap: usa getSession para verificação inicial (síncrona e confiável)
// onAuthChange só é usado para reagir a login/logout após a inicialização
let bootstrapDone = false;

getSession().then(async (session) => {
  bootstrapDone = true;
  if (session) {
    addLogoutButton(session.user.email.split('@')[0]);
    await loadAll();
  } else {
    showLogin();
  }
...
// Reage a mudanças após o bootstrap (login pelo form, logout pelo botão)
onAuthChange(async (session) => {
  if (!bootstrapDone) return; // ignora o disparo inicial do onAuthChange
  if (session) {
    addLogoutButton(session.user.email.split('@')[0]);
